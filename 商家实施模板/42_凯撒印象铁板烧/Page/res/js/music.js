...
var skin_init = function () {
    $('.videolistdiv').css('height', $(window).height() - 150);
...
function musicplaystop() {
    if (Media.paused) {
        Media.play();
        $('.musicplaystop').css('background-image', 'url("res/images/musicplaystop.png")');
    } else {
        Media.pause();
        $('.musicplaystop').css('background-image', 'url("res/images/musicplaystart.png")');
    }
...
function mediaEvent(count) {
    Media.addEventListener("play", function () {
        $.ajax({
            ...
            async: false,
            url: "/action/MusicAction.ashx",
            data: { "SID": SID, "name": dataMusic[currCount].name, "count": count + 1, "option": "OP_UpdateCount" },
            dataType: 'json',
            success: function (data) {
                if (data.ok) {
