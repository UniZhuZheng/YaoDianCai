...
namespace Uni.YDC.Dao.Manager
{
    public class HostDao
    {
         ...
         * */
        public static List<HostEntity> QueryAll()
        {
            using (DbManager dbManager = new DbManager(DBManager.DatabaseId))
            {
                SqlQuery q = new SqlQuery("hosts").Select("id", "hostId", "name", "domain", "ip", "createDate");
                return dbManager.ExecuteList(q).ConvertAll(r => ToHostEntity(r));
            }
        }
        /*
         *HostService. GetHostDomain
         * */
