...
namespace Uni.YDC.Dao.Menu
{
    public class MusicDao
    {
        ...
        public bool Insert(MusicEntity u){
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlInsert i = new SqlInsert("mmusic")
        ...
        public bool Update(string name, string fileName)
        {
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlUpdate u1 = new SqlUpdate("mmusic").
        ...
        public bool Update(string name1, int sort1, string name2, int sort2)
        {
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlUpdate u1 = new SqlUpdate("mmusic").Set("sort", sort1).Where("name", name1);
                bool b1 = dbManager.ExecuteNonQuery(u1) > 0 ? true : false;
                SqlUpdate u2 = new SqlUpdate("mmusic").Set("sort", sort2).Where("name", name2);
        ...
        public bool Delete(string name)
        {
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlDelete q = new SqlDelete("mmusic").Where("name", name);
        ...
        public int Count()
        {
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlQuery q = new SqlQuery("mmusic").SelectMax("sort");
                return dbManager.ExecuteScalar<int>(q);
            }
        }

        public MusicEntity QueryByName(string name)
        {
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlQuery q = new SqlQuery("mmusic").
                    Select("number", "name", "tab", "content", "fileName", "count", "sort", "createDate").
                    Where("name", name).
                    OrderBy("sort", false);
                return dbManager.ExecuteList(q).ConvertAll(r => ToMusicEntity(r)).FirstOrDefault();
            }
        }
        public List<MusicEntity> Query()
        {
            using (DbManager dbManager = new DbManager(SID))
            {
                SqlQuery q = new SqlQuery("mmusic").
                    Select("number", "name", "tab", "content", "fileName", "count", "sort", "createDate").
                    OrderBy("sort",false);
                return dbManager.ExecuteList(q).ConvertAll(r => ToMusicEntity(r));
            }
        }


        private MusicEntity ToMusicEntity(object[] r)
        {
