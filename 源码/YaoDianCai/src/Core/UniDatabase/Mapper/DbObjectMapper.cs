...
namespace UNI.Core.Database.Mapper
{
    public class DbObjectMapper<T> where T : new()
    {
        ...
        private readonly Dictionary<DbColumnAttribute, PropertyInfo> _mappedColumn = new Dictionary<DbColumnAttribute, PropertyInfo>();
        ...
        public bool HasKey { get { return _keys.Count > 0; } }

        public bool HasIdentity { get { return _identity != null; } }
        ...
        private DbObjectMapper()
        {
            ...
            IEnumerable<PropertyInfo> properties =
                typeof(T).GetProperties(BindingFlags.Public | BindingFlags.GetProperty | BindingFlags.Instance).Where(
                    x => x.GetCustomAttributes(typeof(DbNoAttribute), true).Length == 0);
            foreach (PropertyInfo propertyInfo in properties)
            {
                object[] nodataAttr = propertyInfo.GetCustomAttributes(typeof(DbNoAttribute), true);
        ...
        public void InvokeAction(IEnumerable<KeyValuePair<DbColumnAttribute, PropertyInfo>> props, Action<string, object> action, T item)
        {
            if (action != null)
            {
        ...
        public void InvokeAction(IEnumerable<KeyValuePair<DbColumnAttribute, PropertyInfo>> props, Action<string, object> action, params object[] keys)
        {
            if (action != null)
            {
