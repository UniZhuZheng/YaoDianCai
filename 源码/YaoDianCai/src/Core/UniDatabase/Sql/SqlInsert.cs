...
namespace Uni.Core.Database.Sql
{
    public class SqlInsert : ISqlInstruction
    {
        ...
        public object[] GetParameters()
        {
            ...
            List<object> copy = new List<object>(values);
            if (IsIdentityInsert())
            {
        ...
        private bool IsIdentityInsert()
        {
            ...
            if (values[identityPosition] != null && nullValue != null && values[identityPosition].GetType() != nullValue.GetType())
            {
                throw new InvalidCastException(string.Format("Identity null value must be {0} type.", values[identityPosition].GetType()));
            }
