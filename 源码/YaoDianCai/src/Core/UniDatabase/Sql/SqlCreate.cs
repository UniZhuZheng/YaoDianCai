...
namespace Uni.Core.Database.Sql
{
    public abstract class SqlCreate : ISqlInstruction
    {
        ...
        public class Column : SqlCreate
        {
            ...
            public override string ToString(ISqlDialect dialect)
            {
                StringBuilder sql = new StringBuilder().AppendFormat("{0} {1} {2} ", Name, dialect.DbTypeToString(type, size, precision), notNull ? "not null" : "null");
                ...
                if (autoinc)
                {
                    sql.Append(dialect.Autoincrement);
                }
