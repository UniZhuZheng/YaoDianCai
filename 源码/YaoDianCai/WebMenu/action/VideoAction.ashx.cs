...
namespace Uni.WebMenu.action
{
    ...
    public class VideoAction : IHttpHandler
    {
        ...
        private string UpdateCount(HttpContext context)
        {
            ...
            if (!VideoService.UpdateCount(sid, name, count))
            {
                return "{\"ok\":false}";
            }
            return "{\"ok\":true}";
        ...
        private string OP_UpdateInfo(HttpContext context)
        {
            ...
            if (!VideoService.Update(sid, name, number, content, tab))
            {
                return "{\"ok\":false}";
            }
            return "{\"ok\":true}";
        ...
        private string QueryName(HttpContext context)
        {
            string sid = context.Request.Params["SID"];
            string name = context.Request.Params["name"];
            VideoEntity u = VideoService.QueryName(sid,name);
        ...
        private string Delete(HttpContext context)
        {
            string sid = context.Request.Params["SID"];
            string name = context.Request.Params["name"];
            if (!VideoService.Delete(sid, name))
            {
                return "{\"ok\":false}";
            }
            return "{\"ok\":true}";
        ...
        private string Update(HttpContext context)
        {
            ...
            if (!VideoService.Update(sid, name1,sort1,name2,sort2))
            {
                return "{\"ok\":false}";
            }
            return "{\"ok\":true}";
        ...
        private string Insert(HttpContext context)
        {
            ...
            string content = context.Request.Params["content"];
            //判断视频是否存在
            if (VideoService.QueryByName(sid,name) != null)
            {
                return "{\"ok\":false,\"msg\":\"视频名已存在，请重新填写\"}";
            }
            ...
            u.Count = 0;
            u.FileName = name + ".mp4";
            u.Sort = VideoService.Count(sid)+1;

            if (VideoService.Insert(sid, u))
            {
                return "{\"ok\":true,\"msg\":\"视频信息添加成功\"}";
            }
            else {
                return "{\"ok\":false,\"msg\":\"视频信息添加失败\"}";
            }
        ...
        private string SaveRequestFiles(string sid, string name)
        {
            ...
            //商家文件夹路径
            string folderpath = Path.Combine(LocalPath, sid, "Html", "ShopInfo", "video");
