...
function UpdateCartDish(dishName, dishCount) {
    $.getJSON(host+"/jsonpAction/CartAction.ashx?SID="+SID+"&TableName="+TableNum+
            "&DishName="+dishName+"&DishCount="+dishCount+"&option=OP_UpdateCartDishCount&jsoncallback=?",
        function(data){
            if (!data.ok) {
                layer.alert('添加失败。',8);
                return false;
...
function DishUpdate() {
    $(".YDC_products_select_subtract").click(function () {
        ...
        if (parseInt(curcount) > 0) {
            ...
            var curcount = $(this).parent().find('.YDC_products_select_num').text(parseInt(curcount) - 1);
            $('#menu_select_number').text("X" + ToTalorderNum);
            $('#YDC_footer_menu_total_price').text(TotalorderPrice + '元');
        ...
        if (DishInfoMap) {
            ...
            if (dishType == "推荐") {
                $(".YDC_products_item .img").click(function () {
                    var dishName = $(this).attr('name');
                    location.href = "/dishinfo?dishName=" + dishName + "&dishType=特殊二";
                });
            }
            DishUpdate();
        }
...
function body_init() {
    ...
    $('#left_menu').css('height', $(window).height() - 100);
    ...
    $("#popup_tab").click(function () {
        if (TableNum == '餐桌号') {
            layer.alert("请先选择桌号。", 12);
            location.href = "/table";
            return;
        }
        location.href = "/impressTab";
    });
}
...
function data_init() {
    ...
    if (DishTypes) {
        ...
        for (var i = 0; i < DishTypes.length; i++) {
            ...
            if (DishTypes[i].type.indexOf('特殊一') == -1) {
                typeStr += '<div class="menu_type_item ' + ((DishTypes[i].type == dishType) ? 'left_frame_item_click' : '') +
                    '"><span class="type_name">' + (DishTypes[i].type == '特殊二' ? '推荐' : DishTypes[i].type) + '</span></div>';
            } 
        }
    ...
    if (DishInfoMap) {
        ...
        for (var key in DishInfoData) {
            ...
            dishStr += '<div class="YDC_products_item">' +
						...
						'<div class="YDC_products_info">' +
							'<div class="name">' + dish.name + '</div>' +
							'<div class="price" ' + (dishType == "特殊二" ? 'style="display:none;"' : '') + '>' + dish.price + '</div>' +
                            '<div class="price_yuan" ' + (dishType == "特殊二" ? 'style="display:none;"' : '') + '>元</div>' +
						'</div>' +
        ...
        if (dishType == "特殊二") {
            $(".YDC_products_item .img").click(function () {
                var dishName = $(this).attr('name');
                location.href = "/dishinfo?dishName=" + dishName + "&dishType=" + dishType;
            });
        }
    }
    //获取点单总数
